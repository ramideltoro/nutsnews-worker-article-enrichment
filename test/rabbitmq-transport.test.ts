...
describe("RabbitMQ payload transport", () => {
  ...
  it("automatically restores registered enrichment consumers after broker close", async () => {
    vi.useFakeTimers();
    ...
    await transport.consume("enrichment", () => Promise.resolve({
      action: "dlq",
      reason: "not-used"
    }));
    ...
    expect(transport.consumerStatus("enrichment")).toMatchObject({
...
function createFakeBroker(): FakeBroker {
  ...
  return {
    ...
    connect: (url: string): Promise<ChannelModel> => {
      expect(url).toBe("amqp://enrichment:test@example.invalid:5672");
