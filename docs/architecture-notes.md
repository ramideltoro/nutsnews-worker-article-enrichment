...
Local doubles back tests and health probes without production dependencies. Their adapter mode is `local`, so production dependency mode cannot connect the broker or register a consumer with ephemeral acknowledgement state. The production durable adapter factory supplies PostgreSQL state, transactions, and confirmed broker outbox records. All three identify as `production` and must pass bounded permission/connectivity probes before the shadow consumer can run.

The production state adapter returns a fresh opaque token from an atomic claim, compares that token for completion/failure/release, preserves completed records, locks contested inbox rows, computes lease expiry from database-authoritative time, and caps its lease at five minutes. The service-local conformance suite models ambiguous claim and completion responses, stale ownership, completed-record preservation, and lease expiry/reclaim; focused PostgreSQL adapter tests cover commit, rollback, snapshot persistence, and transactional claims.
...
The DNS policy checks protected literal and resolved addresses before the initial fetch and every redirect. The production HTTP dispatcher also enforces the protected-address policy in its socket lookup, so a DNS answer that changes after the earlier policy check still fails before connection. Focused tests prove a metadata-address answer is rejected at that boundary.
