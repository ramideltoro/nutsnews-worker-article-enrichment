...
export interface EnrichmentHttpServer {
...
export function createEnrichmentHttpServer(options: EnrichmentHttpServerOptions): EnrichmentHttpServer {
  ...
  return {
    ...
    url: (path = "/") => {
      ...
      if (!isAddressInfo(address)) {
        throw new Error("Enrichment HTTP server is not listening on a TCP address.");
      }
...
async function routeRequest(
  options: EnrichmentHttpServerOptions,
  request: http.IncomingMessage,
  response: http.ServerResponse
): Promise<void> {
  const url = new URL(request.url ?? "/", "http://localhost");

  if (request.method === "POST" && url.pathname === ENRICHMENT_RECONCILIATION_PATH) {
  ...
  switch (url.pathname) {
    ...
    case "/config-schema":
      writeJson(response, 200, {
        ...
        variables: ENRICHMENT_CONFIG_SCHEMA
      });
      return;
    default:
...
async function handleReconciliationRequest(
  ...
  if (options.reconciler === undefined || options.reconciliationToken === undefined) {
    writeJson(response, 503, {
      ...
      errors: [
        "enrichment reconciliation endpoint is not configured"
      ]
    });
    return;
  }
  ...
  let body: EnrichmentReconciliationRequest;
...
function readJsonBody(request: http.IncomingMessage): Promise<EnrichmentReconciliationRequest> {
  const chunks: Buffer[] = [];
  let totalBytes = 0;
  ...
  return new Promise((resolve, reject) => {
    ...
    request.on("end", () => {
      try {
        ...
        resolve(parsed as unknown as EnrichmentReconciliationRequest);
      } catch {
...
function writeHealth(
  response: http.ServerResponse,
  report: Awaited<ReturnType<EnrichmentService["health"]["liveness"]>>
): void {
